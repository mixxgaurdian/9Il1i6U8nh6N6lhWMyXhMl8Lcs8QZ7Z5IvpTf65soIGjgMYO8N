...
-- CONFIG SYSTEM
local FolderName = "R-Loader/configs/R-LoaderaimDemo"
local FileName = "R-LoaderaimDemo_Settings.json"
...
local Library = (function()
    ...
    local function tween(obj, props, t) TweenService:Create(obj,
            TweenInfo.new(t or 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), props):Play() end

    function UILibrary:CreateWindow(config)
        local title = config.Title or "UI"
        local ScreenGui = create("ScreenGui",
            { Name = "RLoader_Universal_Remaster", Parent = (gethui and gethui()) or CoreGui, ZIndexBehavior = Enum
            .ZIndexBehavior.Sibling, DisplayOrder = 10000, IgnoreGuiInset = true })
        ...
        local Header = create("Frame", { Size = UDim2.new(1, 0, 0, 40), BackgroundColor3 = theme.Header, Parent =
        Container })
        roundify(Header, 10)
        create("Frame",
            { Size = UDim2.new(1, 0, 0, 10), Position = UDim2.new(0, 0, 1, -10), BackgroundColor3 = theme.Header, Parent =
            Header, BorderSizePixel = 0 })

        local Sidebar = create("ScrollingFrame",
            { Size = UDim2.new(0, 140, 1, -40), Position = UDim2.new(0, 0, 0, 40), BackgroundColor3 = theme.Sidebar, Parent =
            Container, ScrollBarThickness = 2, BorderSizePixel = 0 })
        ...
        local ContentArea = create("Frame",
            { Size = UDim2.new(1, -150, 1, -50), Position = UDim2.new(0, 145, 0, 45), BackgroundTransparency = 1, Parent =
            Container })
        ...
        local CloseBtn = create("TextButton",
            { Text = "X", Size = UDim2.new(0, 30, 0, 30), Position = UDim2.new(1, -35, 0, 5), BackgroundTransparency = 1, TextColor3 =
            theme.Error, Font = Enum.Font.GothamBold, TextSize = 14, Parent = Header })
        ...
        UserInputService.InputChanged:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement then dragInput =
                input end end)
        UserInputService.InputChanged:Connect(function(input) if dragging and input == dragInput then
                local delta = input.Position - dragStart; tween(Container,
                    { Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale,
                        startPos.Y.Offset + delta.Y) }, 0.05)
            end end)
        ...
        local NotifyFrame = create("Frame",
            { Size = UDim2.new(0, 250, 1, -150), Position = UDim2.new(1, -260, 0, 20), BackgroundTransparency = 1, Parent =
            ScreenGui, ZIndex = 100 })
        create("UIListLayout",
            { Parent = NotifyFrame, SortOrder = Enum.SortOrder.LayoutOrder, VerticalAlignment = Enum.VerticalAlignment
            .Bottom, Padding = UDim.new(0, 5) })

        local WindowObj = { ScreenGui = ScreenGui, Container = Container }

        function WindowObj:Notify(title, msg)
            local N = create("Frame",
                { Size = UDim2.new(1, 0, 0, 50), BackgroundColor3 = theme.Panel, Parent = NotifyFrame, BackgroundTransparency = 0.1 })
            roundify(N, 6); addStroke(N, theme.Accent)
            create("TextLabel",
                { Text = title, Size = UDim2.new(1, -10, 0, 20), Position = UDim2.new(0, 10, 0, 2), BackgroundTransparency = 1, TextColor3 =
                theme.Accent, Font = Enum.Font.GothamBold, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, Parent =
                N })
            create("TextLabel",
                { Text = msg, Size = UDim2.new(1, -10, 0, 25), Position = UDim2.new(0, 10, 0, 20), BackgroundTransparency = 1, TextColor3 =
                theme.Text, Font = theme.Font, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true, Parent =
                N })
            N.Position = UDim2.new(1, 0, 0, 0)
            tween(N, { Position = UDim2.new(0, 0, 0, 0) }, 0.4)
            task.delay(4,
                function()
                    tween(N, { Position = UDim2.new(1.2, 0, 0, 0), BackgroundTransparency = 1 }, 0.5); task.wait(0.5); N
                        :Destroy()
                end)
        end
        ...
        function WindowObj:CreateCategory(name, icon)
            local TabBtn = create("TextButton",
                { Text = "  " .. (icon or "") .. " " .. name, Size = UDim2.new(1, -10, 0, 30), BackgroundColor3 = theme
                .Sidebar, BackgroundTransparency = 1, TextColor3 = theme.TextDim, Font = theme.Font, TextSize = 13, TextXAlignment =
                Enum.TextXAlignment.Left, Parent = Sidebar, BorderSizePixel = 0, AutoButtonColor = false })
            roundify(TabBtn, 6)

            local TabFrame = create("ScrollingFrame",
                { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Visible = false, ScrollBarThickness = 3, Parent =
                ContentArea, CanvasSize = UDim2.new(0, 0, 0, 0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarImageColor3 =
                theme.Accent })
            create("UIListLayout", { Parent = TabFrame, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 6) })
            create("UIPadding",
                { Parent = TabFrame, PaddingRight = UDim.new(0, 5), PaddingLeft = UDim.new(0, 0), PaddingTop = UDim.new(
                0, 2) })

            TabBtn.MouseButton1Click:Connect(function()
                for _, v in pairs(Sidebar:GetChildren()) do if v:IsA("TextButton") then tween(v,
                            { BackgroundTransparency = 1, TextColor3 = theme.TextDim }, 0.2) end end
                for _, v in pairs(ContentArea:GetChildren()) do v.Visible = false end
                tween(TabBtn, { BackgroundTransparency = 0.8, BackgroundColor3 = theme.Accent, TextColor3 = theme.Accent },
                    0.2); TabFrame.Visible = true
            end)
            ...
            function TabObj:Label(text) create("TextLabel",
                    { Text = text, Size = UDim2.new(1, 0, 0, 20), BackgroundTransparency = 1, TextColor3 = theme.Accent, Font =
                    Enum.Font.GothamBold, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent = TabFrame }) end
            ...
            function TabObj:Slider(text, min, max, default, callback)
                local Frame = create("Frame",
                    { Size = UDim2.new(1, 0, 0, 40), BackgroundColor3 = theme.ButtonBg, Parent = TabFrame }); roundify(
                Frame, 4)
                create("TextLabel",
                    { Text = text, Size = UDim2.new(1, -10, 0, 15), Position = UDim2.new(0, 10, 0, 5), BackgroundTransparency = 1, TextColor3 =
                    theme.Text, Font = theme.Font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent =
                    Frame })
                local ValueLabel = create("TextLabel",
                    { Text = tostring(default), Size = UDim2.new(0, 40, 0, 15), Position = UDim2.new(1, -45, 0, 5), BackgroundTransparency = 1, TextColor3 =
                    theme.TextDim, Font = theme.Font, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Right, Parent =
                    Frame })
                local SliderBar = create("TextButton",
                    { Text = "", Size = UDim2.new(1, -20, 0, 4), Position = UDim2.new(0, 10, 0, 28), BackgroundColor3 =
                    theme.Panel, AutoButtonColor = false, Parent = Frame }); roundify(SliderBar, 2)
                local Fill = create("Frame",
                    { Size = UDim2.new((default - min) / (max - min), 0, 1, 0), BackgroundColor3 = theme.Accent, Parent =
                    SliderBar }); roundify(Fill, 2)
                local dragging = false
                local function update(input)
                    local pos = math.clamp((input.Position.X - SliderBar.AbsolutePosition.X) / SliderBar.AbsoluteSize.X,
                        0, 1)
                    tween(Fill, { Size = UDim2.new(pos, 0, 1, 0) }, 0.05)
                    local val = math.floor(min + ((max - min) * pos)); if max < 5 then val = math.floor((min + ((max - min) * pos)) *
                        100) / 100 end
                    ValueLabel.Text = tostring(val); callback(val)
                end
                SliderBar.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            function TabObj:Dropdown(text, callback)
                local Frame = create("Frame",
                    { Size = UDim2.new(1, 0, 0, 28), BackgroundColor3 = theme.ButtonBg, Parent = TabFrame, ClipsDescendants = true }); roundify(
                Frame, 4)
                local Header = create("TextButton",
                    { Text = text .. " ▼", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, TextColor3 = theme
                    .Text, Font = theme.Font, TextSize = 12, Parent = Frame })
                local List = create("ScrollingFrame",
                    { Size = UDim2.new(1, 0, 0, 100), Position = UDim2.new(0, 0, 0, 28), BackgroundTransparency = 1, Parent =
                    Frame, CanvasSize = UDim2.new(0, 0, 0, 0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarThickness = 2 }); create(
                    ...
                    for _, p in pairs(Players:GetPlayers()) do
                        if p ~= LocalPlayer then
                            local OptBtn = create("TextButton",
                                { Text = p.Name, Size = UDim2.new(1, 0, 0, 25), BackgroundColor3 = theme.Panel, BackgroundTransparency = 0.5, TextColor3 =
                                theme.TextDim, Font = theme.Font, TextSize = 11, Parent = List })
                            OptBtn.MouseButton1Click:Connect(function()
                                callback(p.Name); Header.Text = text .. " [" .. p.Name .. "] ▼"; tween(Frame,
                                    { Size = UDim2.new(1, 0, 0, 28) }, 0.2)
                            end)
                        end
                    end
                end
                ...
                Header.MouseButton1Click:Connect(function()
                    open = not open; if open then
                        Refresh(); tween(Frame, { Size = UDim2.new(1, 0, 0, 130) }, 0.2)
                    else tween(Frame, { Size = UDim2.new(1, 0, 0, 28) }, 0.2) end
                end)
            end

            function TabObj:Binder(text, defaultKey, callback)
                local Frame = create("Frame",
                    { Size = UDim2.new(1, 0, 0, 28), BackgroundColor3 = theme.ButtonBg, Parent = TabFrame }); roundify(
                Frame, 4)
                create("TextLabel",
                    { Text = text, Size = UDim2.new(1, -100, 1, 0), Position = UDim2.new(0, 10, 0, 0), BackgroundTransparency = 1, TextColor3 =
                    theme.Text, Font = theme.Font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent =
                    Frame })
                local BindBtn = create("TextButton",
                    { Text = defaultKey, Size = UDim2.new(0, 80, 0, 20), Position = UDim2.new(1, -90, 0.5, -10), BackgroundColor3 =
                    theme.Panel, TextColor3 = theme.TextDim, Font = theme.Font, TextSize = 11, Parent = Frame }); roundify(
                BindBtn, 4)
...
local AimKeyBtn
AimKeyBtn = CombatTab:Button(
"Aim Key: " .. (Config.Aimbot.AimKey == "MouseButton1" and "Left Click (MB1)" or "Right Click (MB2)"), function()
...
CombatTab:Dropdown("Select to Whitelist", function(name)
    Config.Aimbot.Whitelist[name] = true; SaveConfig()
end)
CombatTab:Button("Clear Whitelist",
    function()
        Config.Aimbot.Whitelist = {}; Window:Notify("System", "Whitelist Cleared")
    end)
...
ObjBtn = VisualsTab:Button("Obj Mode: " .. Config.ESP.ObjectMode, function()
    ...
    elseif Config.ESP.ObjectMode == "Tools" then
        Config.ESP.ObjectMode = "NPCs"
    elseif Config.ESP.ObjectMode == "NPCs" then
        ...
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("Head") and plr.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if hum and hum.Health > 0 then
                    ...
                    if not hl then
                        hl = Instance.new("Highlight", ESPFolder); hl.Name = hlName; hl.FillTransparency = 0.5; hl.OutlineTransparency = 0
                    end
                    hl.Adornee = char
                    ...
                    if Config.Aimbot.TeamCheck and plr.Team == LocalPlayer.Team then
                        ...
                        hl.OutlineColor = Color3.fromRGB(Config.ESP.Outline.R, Config.ESP.Outline.G, Config.ESP.Outline
                        .B)
                    end
                    ...
                    if Config.ESP.ShowNames then
                        if not tag then
                            tag = Instance.new("BillboardGui", ESPFolder); tag.Name = tagName; tag.AlwaysOnTop = true; tag.Size =
                            UDim2.new(0, 200, 0, 50); tag.StudsOffset = Vector3.new(0, -5, 0)
                            local label = Instance.new("TextLabel", tag); label.BackgroundTransparency = 1; label.Size =
                            UDim2.new(1, 0, 1, 0); label.TextColor3 = Color3.new(1, 1, 1); label.TextSize = 13; label.Font =
                    ...
                    if Config.ESP.Health and onScreen and hum then
                        if not barOutline then
                            barOutline = Instance.new("Frame", ESP_2D); barOutline.Name = barName; barOutline.BorderSizePixel = 1; barOutline.BorderColor3 =
                            Color3.new(0, 0, 0); barOutline.BackgroundColor3 = Color3.fromRGB(60, 60, 60); barOutline.ZIndex = 2
                            local fill = Instance.new("Frame", barOutline); fill.Name = "Fill"; fill.BorderSizePixel = 0; fill.BackgroundColor3 =
                            Color3.fromRGB(0, 255, 0); fill.AnchorPoint = Vector2.new(0, 1); fill.Position = UDim2.new(0,
                                0, 1, 0); fill.ZIndex = 3
                        end
                        local headPos = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.5, 0))
                        local legPos = Camera:WorldToViewportPoint(root.Position - Vector3.new(0, 3, 0))
                    ...
                    if Config.ESP.Tracers and onScreen then
                        if not line then
                            line = Instance.new("Frame", ESP_2D); line.Name = lineName; line.BorderSizePixel = 0; line.AnchorPoint =
                            Vector2.new(0.5, 0.5)
                        end
        ...
        for _, object in pairs(InteractableCache) do
            if object and object.Parent then
                ...
                if Config.ESP.ObjectMode == "All" then
                    show = true
                elseif Config.ESP.ObjectMode == "Tools" and (object:IsA("Tool") or object:FindFirstChildWhichIsA("Tool")) then
                    show = true
                elseif Config.ESP.ObjectMode == "Interactable" and (object:FindFirstChildWhichIsA("ProximityPrompt", true) or object:FindFirstChildWhichIsA("ClickDetector", true)) then
                    show = true
                elseif Config.ESP.ObjectMode == "NPCs" and object:IsA("Model") and object:FindFirstChild("Humanoid") then
                    show = true
                end
                ...
                if show then
                    ...
                    if not hl then
                        hl = Instance.new("Highlight", ESPFolder); hl.Name = objName; hl.FillTransparency = 0.5; hl.OutlineTransparency = 0
                    end
                    hl.Adornee = object
                    hl.FillColor = object:FindFirstChild("Humanoid") and Color3.fromRGB(255, 170, 0) or
                    Color3.new(0, 1, 0)
                else
...
RunService.RenderStepped:Connect(function()
    if aiming and Config.Aimbot.Enabled then
                        ...
                        if (targetPart.Position - camPos).Magnitude <= Config.Aimbot.Range then
                            ...
                            if vis then
                                ...
                                if dist < maxDist then
                                    maxDist = dist; closest = targetPart
                                end
                            end
                        end
                    end
        ...
        if not closest then
            for _, p in pairs(Players:GetPlayers()) do
                local isTeammate = Config.Aimbot.TeamCheck and p.Team and LocalPlayer.Team and p.Team == LocalPlayer
                .Team
                ...
                if p ~= LocalPlayer and p.Character and not Config.Aimbot.Whitelist[p.Name] and not isTeammate then
                    ...
                    if isTargetValid then
                        ...
                        elseif Config.Aimbot.TargetMode == "Body" then
                            targetP = p.Character:FindFirstChild("HumanoidRootPart") or
        ...
        if closest then
            ...
            mousemoverel((pos.X - mousePos.X) / Config.Aimbot.Smoothness, (pos.Y - mousePos.Y) / Config.Aimbot
            .Smoothness)
        end
    end
end)
